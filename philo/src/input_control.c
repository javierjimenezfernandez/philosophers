...
/*   Updated: 2025/09/28 19:34:38 by javjimen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	is_out_of_range(char *num, char *range, t_arguments argument)
{
	...
	if (ft_strlen(num) == ft_strlen(range))
	{
		if (num[ft_strlen(num) - 1] > range[ft_strlen(range) - 1])
			return (true);
	}
...
t_init_cond	parse_input(int argc, char **argv)
{
	...
	init_cond.number_of_philosophers = ft_atoui(argv[number_of_philosophers]);
	init_cond.time_to_die = ft_atoui(argv[time_to_die]);
	init_cond.time_to_eat = ft_atoui(argv[time_to_eat]);
	init_cond.time_to_sleep = ft_atoui(argv[time_to_sleep]);
	if (argc == 5)
		init_cond.number_of_times_each_philosopher_must_eat = 0;
	else if (argc == 6)
	{
		init_cond.number_of_times_each_philosopher_must_eat = \
			ft_atoui(argv[number_of_times_each_philosopher_must_eat]);
